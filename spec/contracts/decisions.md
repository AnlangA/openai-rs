...
- Sources: round-11 audit observations (round-11 review notes items 11-10 and the per-domain notes).
...
- Sources: round-12 item 12-04 (round-12 review notes) — decoding already rejects rows with an empty `custom_id` (`BatchCustomId::new` refuses exactly-empty strings with `BatchValidationError::EmptyCustomId`, no whitespace trimming), but the writer checked nothing, so a decoded-and-replayed or hand-built `BatchLine` could re-encode an empty id the batch API rejects (unique non-empty `custom_id` is a documented batch requirement).
...
- Sources: round-12 audit observations (round-12 review notes item 12-06 and the per-domain notes).
...
- Sources: round-13 informational findings (round-13 review notes informational findings, domains B/C/D/F/G/H/J/L/M/N/O/P).
...
- Sources: round-14 items 14-C-1/14-C-2 (round-14 review notes) — python ships `ChatCompletionStreamManager`/`ChatCompletionStream` (`lib/streaming/chat/_completions.py:46` `until_done`/`get_final_completion`/snapshot; fold engine `lib/_deltas.py:6` `accumulate_delta` merges tool_calls by chunk index) and node `lib/ChatCompletionStream.ts`; the crate had `ResponseAccumulator` on the Responses channel but nothing on chat (internal asymmetry); the chat and legacy-completions `event: error` remote-error lanes had zero test coverage.
...
- Sources: round-14 informational findings (round-14 review notes informational findings).
...
- Sources: round-15 informational findings (round-15 review notes informational findings).
...
- Sources: round-16 informational findings (round-16 review notes informational findings).
...
- Sources: round-17 findings (round-17 review notes) — the ledgered follow-ups from D0277/D0279/D0280/D0282/D0281's reports plus the twelve-domain test-gap census (431/431 ledger Tests references verified present and runnable).
...
- Sources: round-17 informational findings (round-17 review notes informational findings).
...
- Sources: round-18 verification (round-18 review notes).
...
## D0292 — Graded diagnostics and opt-in raw JSON through RUST_LOG

- Status: accepted
- Reviewed: 2026-09-11
- Scope: Platform HTTP transport diagnostics, shared tracing helpers, terminal examples
- Sources: user request to inspect original JSON for an OpenAI-compatible response decode failure, use standard `RUST_LOG`, and add useful graded logs; https://docs.rs/tracing-subscriber/0.3.23/tracing_subscriber/filter/struct.EnvFilter.html.
- Decision: ERROR records terminal HTTP/transport/body/decode failures with safe metadata; WARN records retries/deadlines; INFO records accepted response headers with operation/request ID/status/retries/header latency; DEBUG records attempts, remaining timeout, body byte count and authentication invalidation. The Platform JSON transport adds raw request/response TRACE events under `openai_rs_client::http_body`, including response bytes before deserialization and bounded intermediate HTTP retry bodies. The normal six-field request span remains unchanged. All examples read `RUST_LOG` through an application-owned subscriber, defaulting to WARN and writing to stderr; no SDK-specific environment flag, public builder flag or library-owned subscriber is added.
- Overrides: D0231's blanket prohibition on body events is narrowed by the user's explicit raw-JSON diagnostic request. INFO/DEBUG/WARN/ERROR remain metadata-only. Authentication headers/exchange bodies, multipart/binary payloads and SSE/WS frames are excluded; application secrets explicitly embedded in raw JSON are not redacted. Existing body limits and total retry budgets remain in force.
- Tests: trace capture regressions for INFO context with DEBUG disabled, DEBUG body exclusion, TRACE request/response bytes, decode-failure ordering and path, ERROR body-value exclusion, HTTP retry diagnostics, and unchanged authentication/SSE leak boundaries; subprocess example smoke checks exercise actual `RUST_LOG` filtering and stderr output.

## D0293 — Preserve omitted cache-write usage from compatible Responses providers

- Status: accepted
- Reviewed: 2026-09-11
- Scope: `InputTokensDetails.cache_write_tokens` across Responses, compact and beta Responses
- Sources: user-supplied DeepSeek HTTP 200 response with `usage.input_tokens_details` equal to `{"cached_tokens":0}`; https://api-docs.deepseek.com/api/create-response/ documents and demonstrates this shape.
- Decision: store `cache_write_tokens` as `Omittable<u64>` and expose it as `Option<u64>`. Missing values remain absent on serialization, explicit zero and positive counts retain their values, and invalid present values are rejected. No synthetic zero, provider hostname detection, or catch-all relaxation of the response schema is introduced.
- Overrides: D0092's required cache-write count is relaxed for the documented compatible-provider response. The pinned upstream OpenAPI bytes remain unchanged. Other required usage fields remain required.
- Impact: the getter changes from `u64` to `Option<u64>`; callers must handle an unreported count. Normal OpenAI responses return `Some(count)`. No release version is changed in this fix.
- Tests: `response_usage_preserves_reported_and_omitted_cache_write_tokens`, `deepseek_response_without_cache_write_tokens_decodes_and_replays_history`; the raw logging failure fixture now uses an explicitly invalid null count rather than a valid omission.

## D0294 — English project text and enforced Rustdoc conventions

- Status: accepted
- Reviewed: 2026-09-11
- Scope: project-owned source text, examples, fixtures, review reports, and Rust API documentation
- Sources: user request to replace Chinese project text with English and document Rust APIs according to Rustdoc conventions; https://doc.rust-lang.org/rustdoc/how-to-write-documentation.html and https://rust-lang.github.io/api-guidelines/documentation.html.
