...
pub enum AdminApiKeyError {
    /// The Administration API key is empty.
    #[error("the Administration API key is empty")]
    Empty,
    /// The Administration API key has surrounding whitespace.
    #[error("the Administration API key has surrounding whitespace")]
    SurroundingWhitespace,
    /// The Administration API key contains whitespace.
    #[error("the Administration API key contains whitespace")]
    Whitespace,
    /// The Administration API key contains a control character.
    #[error("the Administration API key contains a control character")]
    ControlCharacter,
    /// The Administration API key contains non-ASCII characters.
    #[error("the Administration API key contains non-ASCII characters")]
    NonAscii,
    /// The Administration API key cannot be represented as an HTTP header.
...
/// Request-body encoding required by an Administration operation.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum AdminRequestEncoding {
    /// The operation sends no request body.
    None,
    /// The operation sends a JSON request body.
    Json,
}

/// Response-body decoder selected by an Administration operation.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub trait AdminOperation: admin_operation_private::Sealed + Send + Sync + 'static {
    /// Typed payload accepted by this operation.
    type Request: Serialize + Send + Sync + 'static;
    /// Typed payload decoded from a successful response.
    type Response: DeserializeOwned + Send + 'static;
    /// Stable operation identifier from the pinned API contract.
    const ID: &'static str;
    /// HTTP method required by the operation.
    const METHOD: Method;
    /// HTTP route template, with named placeholders for path parameters.
    const ROUTE: &'static str;
    /// Credential scope required to execute the operation.
    const AUTH: AdminAuthScope = AdminAuthScope::Admin;
    /// Encoding used for the request body.
    const REQUEST_ENCODING: AdminRequestEncoding;
    /// Decoder used for the response body.
    const RESPONSE_MODE: AdminResponseMode;
    /// HTTP status codes accepted as successful responses.
    const SUCCESS_STATUSES: &'static [StatusCode];
    /// Media types accepted for successful response bodies.
    const RESPONSE_CONTENT_TYPES: &'static [&'static str];
    /// Name of the Rust request-payload type.
    const REQUEST_TYPE: &'static str;
    /// Name of the Rust response-payload type.
    const RESPONSE_TYPE: &'static str;
    /// Pinned schema references corresponding to the request payload.
    const REQUEST_SCHEMA_REFS: &'static [&'static str];
    /// Pinned schema references corresponding to the response payload.
...
macro_rules! admin_operation {
    ($name:ident, $id:literal, $method:ident, $route:literal, $request:ty, $response:ty, $encoding:ident, $mode:ident, $request_refs:expr, $response_refs:expr) => {
        #[doc = concat!("The `", $id, "` operation: `", stringify!($method), " ", $route, "`.")]
        ...
        impl $name {
            /// Pinned HTTP, encoding, and schema metadata for this operation.
...
pub struct AdminClientOperationContract {
    ...
    /// Rust type name used for the request payload.
    pub request_type: &'static str,
    /// Rust type name used for the response payload.
...
impl AdminClient {
    ...
    /// # Errors
    ///
    /// Returns a configuration or HTTP client initialization error if the Administration client
    /// cannot be constructed with the supplied key.
    ...
    /// Provides access to API-key administration operations.
    #[must_use]
    pub fn api_keys(&self) -> AdminApiKeys {
        AdminApiKeys(self.clone())
    }

    /// Provides access to audit-log retrieval operations.
    #[must_use]
    pub fn audit_logs(&self) -> AdminAuditLogs {
        AdminAuditLogs(self.clone())
    }

    /// Provides access to certificate administration operations.
    #[must_use]
    pub fn certificates(&self) -> AdminCertificates {
        AdminCertificates(self.clone())
    }

    /// Provides access to data-retention administration operations.
    #[must_use]
    pub fn data_retention(&self) -> AdminDataRetention {
        AdminDataRetention(self.clone())
    }

    /// Provides access to group administration operations.
    #[must_use]
    pub fn groups(&self) -> AdminGroups {
        AdminGroups(self.clone())
    }

    /// Provides access to user administration operations.
    #[must_use]
    pub fn users(&self) -> AdminUsers {
        AdminUsers(self.clone())
    }

    /// Provides access to role administration operations.
    #[must_use]
    pub fn roles(&self) -> AdminRoles {
        AdminRoles(self.clone())
    }

    /// Provides access to invitation administration operations.
    #[must_use]
    pub fn invites(&self) -> AdminInvites {
        AdminInvites(self.clone())
    }

    /// Provides access to project administration operations.
    #[must_use]
    pub fn projects(&self) -> AdminProjects {
        AdminProjects(self.clone())
    }

    /// Provides access to usage and cost retrieval operations.
    #[must_use]
...
impl AdminClientBuilder {
    /// Creates an Administration client builder with the supplied admin API key.
    #[must_use]
    ...
    /// Validates the configuration and constructs an Administration client.
    ///
    /// # Errors
    ///
    /// Returns an error for invalid endpoint, timeout, body-limit, authentication-header, or TLS
    /// settings, or if the HTTP client cannot be initialized.
...
impl<O: AdminOperation> AdminRequest<O> {
    ...
    /// # Errors
    ///
    /// Returns an error if the value cannot be encoded as a valid path segment or query for the
    /// operation contract.
...
/// Organization administration operations for API keys.
#[derive(Clone, Debug)]
...
impl AdminApiKeys {
    ...
    /// Retrieves a single organization API key.
    ///
    /// # Errors
    ///
    /// Returns a client error if request preparation, authentication, transport, service execution,
...
/// Retrieves organization audit-log entries and their actor metadata.
#[derive(Clone, Debug)]
pub struct AdminAuditLogs(AdminClient);

impl AdminAuditLogs {
    /// Lists user actions and configuration changes within this organization.
    ///
    /// # Errors
    ///
    /// Returns a client error if request preparation, authentication, transport, service execution,
...
/// Administration operations for trusted certificates and their activation scopes.
#[derive(Clone, Debug)]
pub struct AdminCertificates(AdminClient);

impl AdminCertificates {
    /// Lists uploaded certificates for this organization.
    ///
    /// # Errors
    ///
    /// Returns a client error if request preparation, authentication, transport, service execution,
    ...
    /// Retrieves certificate metadata using the default retrieval options.
    ///
    /// # Errors
    ///
    /// Returns a client error if request preparation, authentication, transport, service execution,
    ...
    /// Modifies a certificate. Note that only the name can be modified.
    ///
    /// # Errors
    ///
    /// Returns a client error if request preparation, authentication, transport, service execution,
    ...
    /// Activates certificates at the organization level.
    ///
    ...
    /// Deactivates certificates at the organization level.
    ///
...
/// Reads and updates organization or project data-retention settings.
#[derive(Clone, Debug)]
...
/// Administration operations for groups, group members, and role assignments.
#[derive(Clone, Debug)]
...
/// Administration operations for organization users and their memberships.
#[derive(Clone, Debug)]
...
/// Administration operations for custom roles and role assignments.
#[derive(Clone, Debug)]
...
/// Creates, lists, retrieves, and deletes organization invitations.
#[derive(Clone, Debug)]
...
impl AdminInvites {
    ...
    /// Creates an invite for a user to the organization. The invite must be accepted by the user
    ...
    /// Deletes an invite. If the invite has already been accepted, it cannot be deleted.
    ///
    /// # Errors
    ///
    /// Returns a client error if request preparation, authentication, transport, service execution,
...
/// Administration operations for projects and their scoped resources.
#[derive(Clone, Debug)]
...
impl AdminProjects {
    ...
    /// Creates a new project in the organization. Projects can be created and archived, but cannot
    /// be deleted.
    ///
    /// # Errors
    ///
...
/// Retrieves organization usage and cost aggregates.
#[derive(Clone, Debug)]
pub struct AdminUsage(AdminClient);

macro_rules! usage_method {
    ($name:ident, $operation:ty) => {
        #[doc = concat!("Retrieves organization usage through `", stringify!($operation), "`.")]
        ///
        /// # Errors
        ///
        /// Returns an error if query encoding, authentication, transport, the
        /// service response, or typed response decoding fails.
        pub async fn $name(
            &self,
